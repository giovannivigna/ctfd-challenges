...
def add_file():
    try:
        ...
        content = base64.b64decode(data.get('fileContent'))

        with open(f"/tmp/app/{filename}", mode="wb") as f:
            f.write(content)
...
@app.route('/documentfile', methods=['POST'])
@login_required
...
def remove_document(document_id):
    logging.debug(f"Removing document {document_id}")
    try:
        document = Document.query.filter_by(id=document_id, user_id=current_user.id).first()
...
if __name__ == '__main__':
    app.run(host='0.0.0.0', port=8862)
