...
def main():
    ...
    try:
        ...
        print(f"Executing the program with payload [{payload}] of length {num_bytes}...", flush=True)
        # Execute the vulnerable program with the payload via stdin
        #process = subprocess.run(sys.argv[2], input=payload.encode(), 
        #                        preexec_fn=os.setsid, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        process = subprocess.Popen([sys.argv[2]],  
                                    stdin=subprocess.PIPE,  # Allows writing input
        ...
        print(f"Program terminated and returned {process.returncode}!", flush=True)
